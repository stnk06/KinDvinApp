...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupGestureDetectors() {
        ...
        rootContainer.setOnTouchListener((v, event) -> {
            View uiPanel = findViewById(R.id.players_card);
            if (uiPanel != null && event.getX() < uiPanel.getLeft()) {
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            gameContainer.setScaleX(scaleFactor);
            gameContainer.setScaleY(scaleFactor);
            return true;
        }
    }
    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            gameContainer.setTranslationX(gameContainer.getTranslationX() - distanceX);
            gameContainer.setTranslationY(gameContainer.getTranslationY() - distanceY);
            return true;
        }
    ...
    private void resetView() {
        scaleFactor = 1.0f;
        ObjectAnimator scaleX = ObjectAnimator.ofFloat(gameContainer, "scaleX", 1.0f);
        ObjectAnimator scaleY = ObjectAnimator.ofFloat(gameContainer, "scaleY", 1.0f);
        ObjectAnimator translationX = ObjectAnimator.ofFloat(gameContainer, "translationX", 0f);
        ObjectAnimator translationY = ObjectAnimator.ofFloat(gameContainer, "translationY", 0f);
    ...
    private List<PointF> generateCellCoordinates(int width, int height) {
        ...
        float paddingHorizontal = cellDiameter * 0.5f;
        float paddingVertical = cellDiameter * 0.5f;

        float workingWidth = width - (2 * paddingHorizontal);
        float workingHeight = height - (2 * paddingVertical);

        float horizontalStep = (cols > 1) ? workingWidth / (cols - 1) : 0;
        float verticalStep = (rows > 1) ? workingHeight / (rows - 1) : 0;
        ...
        for (int i = 0; i < BOARD_SIZE; i++) {
            ...
            if (currentRow % 2 != 0) {
                currentCol = cols - 1 - currentCol;
            }

            float centerX = paddingHorizontal + currentCol * horizontalStep;
            float centerY = paddingVertical + currentRow * verticalStep;

            float x = centerX - (cellDiameter / 2);
            float y = centerY - (cellDiameter / 2);
    ...
    private void animateMove(Team team, int from, int to, boolean isForward) {
        ...
        animatedPawn.setScaleX(1f);
        animatedPawn.setScaleY(1f);
        animatedPawn.setTranslationY(0f);
        ...
        moveAnimatorSet.playTogether(xAnimator, yAnimator, rotation);
        moveAnimatorSet.setDuration(800);
        moveAnimatorSet.setInterpolator(new AccelerateDecelerateInterpolator());

        moveAnimatorSet.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                ObjectAnimator squashY = ObjectAnimator.ofFloat(animatedPawn, View.SCALE_Y, 1f, 0.8f);
                ObjectAnimator squashX = ObjectAnimator.ofFloat(animatedPawn, View.SCALE_X, 1f, 1.2f);
                squashY.setDuration(100);
                squashX.setDuration(100);

                ObjectAnimator stretchY = ObjectAnimator.ofFloat(animatedPawn, View.SCALE_Y, 0.8f, 1.1f);
                ObjectAnimator stretchX = ObjectAnimator.ofFloat(animatedPawn, View.SCALE_X, 1.2f, 0.9f);
                ObjectAnimator bounceUp = ObjectAnimator.ofFloat(animatedPawn, View.TRANSLATION_Y, 0, -25f);
                stretchY.setDuration(150);
                stretchX.setDuration(150);
                bounceUp.setDuration(150);

                ObjectAnimator settleY = ObjectAnimator.ofFloat(animatedPawn, View.SCALE_Y, 1.1f, 1f);
                ObjectAnimator settleX = ObjectAnimator.ofFloat(animatedPawn, View.SCALE_X, 0.9f, 1f);
                ObjectAnimator bounceDown = ObjectAnimator.ofFloat(animatedPawn, View.TRANSLATION_Y, -25f, 0f);
                settleY.setDuration(200);
                settleX.setDuration(200);
                bounceDown.setDuration(200);

                AnimatorSet landingSet = new AnimatorSet();
                landingSet.play(squashY).with(squashX);
                landingSet.play(stretchY).with(stretchX).with(bounceUp).after(squashY);
                landingSet.play(settleY).with(settleX).with(bounceDown).after(stretchY);
    ...
    private void handleSpecialCell(Team team) {
        ...
        switch (type) {
            case ATTACK:
                SoundManager.playAttack(this);
                animationHandler.postDelayed(() -> showAttackDialog(team), 500);
                return;
            case CHALLENGE:
                SoundManager.playChallenge(this);
